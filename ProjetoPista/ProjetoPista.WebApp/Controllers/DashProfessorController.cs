...
namespace ProjetoPista.WebApp.Controllers
{
    ...
    public class DashProfessorController : Controller
    {
        ...
        {
            var professor = _professorBusiness.SelecionarProfessorFull(id);

            return PartialView("ProfInfo",professor);
        }
        public IActionResult PistasProf(int id)
        {
            var pistaProf = _professorBusiness.FiltrarPistasProf(id);
            ViewBag.id_pista = null;
            ViewBag.Estado = _pistaBusiness.ComboEstado();
            ViewBag.Cidade = _pistaBusiness.ComboCidade();
            ...
            ViewBag.ProfessorModel = id;

            return View("../Home/Index", pistaProf);
        }
