...
namespace ProjetoPista.WebApp.Controllers
{
    ...
    public class AprovacoesController : Controller
    {
        ...
        [HttpPost]
        public IActionResult SalvarUser(PistaCadastroUserDto pistaDto)
        {
            pistaDto.nm_imagens = UploadedFile(pistaDto);
            ...
            if (pistaDto.fl_vert) { modalidadesSelecionadas.Add(1); }
            if (pistaDto.fl_street) { modalidadesSelecionadas.Add(2); }
            if (pistaDto.fl_half) { modalidadesSelecionadas.Add(3); }
            if (pistaDto.fl_hill) { modalidadesSelecionadas.Add(5); }
            if (pistaDto.fl_plaza) { modalidadesSelecionadas.Add(6); }
            if (pistaDto.fl_facil) { niveisSelecionados.Add(1); }
            if (pistaDto.fl_facilmedio) {niveisSelecionados.Add(2); }
            if (pistaDto.fl_medio) { niveisSelecionados.Add(3); niveisSelecionados.Add(2); }
            if (pistaDto.fl_mediodificil) { niveisSelecionados.Add(3);}
            if (pistaDto.fl_dificil) { niveisSelecionados.Add(5); niveisSelecionados.Add(4); }

            pistaDto.modalidadesSelecionadas = modalidadesSelecionadas;
            pistaDto.niveisSelecionados = niveisSelecionados;
        ...
        private string[] UploadedFile(PistaCadastroUserDto model)
        {
            string uniqueFileName = null;
            List<string> nomes = new List<string>();
            if (model.imagens_pista != null)
        ...
        public IActionResult VerificaNome(string nm_pista,int id_pista)
        {
            ...
            {
                return Json(data: "Esse nome já está sendo utilizado, tente colocar mais alguma informação de Local. Ex: Pista Tal (Campinas)");
            }
