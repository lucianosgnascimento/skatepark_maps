...
namespace ProjetoPista.Business
{
    public class PistaBusiness : IPistaBusiness
    {
        ...
        public IEnumerable<PistaDto> FiltrarPistas()
        {
            ...
            var PistaEndereco = pista.GetAll().Join(endereco.GetAll(),
                        ...
                        new PistaDto
                        {
                            ...
                            nm_imagens = this._unitOfWork.FotosRepository.Get().Where(o => o.id_pista.Equals(pist.ID_pista)).Select(n => n.nm_foto).ToArray(),
                        ...
                        .Where(o => o.id_status_aprovacao.Equals(1))
        ...
        public IEnumerable<PistaDto> FiltrarPistas(PistaFiltroDto pistaFiltro)
        {
            ...
            var PistaEndereco = pista.GetAll().Join(endereco.GetAll(),
                        ...
                        new PistaDto
                        {
                            ...
                            nm_imagens = this._unitOfWork.FotosRepository.Get().Where(o => o.id_pista.Equals(pist.ID_pista)).Select(n => n.nm_foto).ToArray(),
                            modalidadesSelecionadas = this._unitOfWork.ModalidadePistaRepository.Get(null, o => o.OrderBy(u => u.id_modalidade_pista)).Where(o => o.id_pista.Equals(pist.ID_pista)).Select(s => s.id_modalidade).ToList(),
                            niveisSelecionados = this._unitOfWork.NiveisDificuldadePistaRepository.Get(null, o => o.OrderBy(u => u.id_nivel_dificuldade_pista)).Where(o => o.id_pista.Equals(pist.ID_pista)).Select(s => s.id_nivel_dificuldade).ToList()
            ...
            if (pistaFiltro.modalidadesSelecionadas.Count() != 0)
            {

                var modalidadePista = this._unitOfWork.ModalidadePistaRepository;
                var vizualizacao = 0;

                //for (int i = 0; i < pistaFiltro.modalidadesSelecionadas.Count(); i++)
                //{
                    /*
                    PistaEndereco = PistaEndereco.Join(modalidadePista.GetAll(),
                        ...
                        new PistaDto
                        {
                            ...
                            modalidadesSelecionadas = pist.modalidadesSelecionadas

                        })
                    //ok.Where(m => m.modalidadesSelecionadas.Any(t => t == pistaFiltro.modalidadesSelecionadas.ToList()[0]))
                    //ok2.Where(o => pistaFiltro.modalidadesSelecionadas.ToList().Contains(o.id_modalidade))
                        .Where(o => pistaFiltro.modalidadesSelecionadas.ToList().Contains(o.id_modalidade)) 
                    ;
                    */
                    PistaEndereco = PistaEndereco
                    .Where(m => m.modalidadesSelecionadas.Any(t => t == pistaFiltro.modalidadesSelecionadas.ToList()[0]))
                    .Select(s => new PistaDto
                    {
                        id_pista = s.id_pista,
                        id_endereco = s.id_endereco,
                        ...
                        nr_latitude = s.nr_latitude,
                        nr_longitude = s.nr_longitude,
                        modalidadesSelecionadas = s.modalidadesSelecionadas
                    })
                    ;
                //}
            ...
            if (pistaFiltro.niveisSelecionados.Count() != 0)
            {
                var nivelDificuldadePista = this._unitOfWork.NiveisDificuldadePistaRepository;
                PistaEndereco = PistaEndereco
                    .Where(m => m.niveisSelecionados.Any(t => t == pistaFiltro.niveisSelecionados.ToList()[0]))
                    .Select(s => new PistaDto
                    {
                        id_pista = s.id_pista,
                        id_endereco = s.id_endereco,
                        ...
                        nr_longitude = s.nr_longitude,
                        modalidadesSelecionadas = s.modalidadesSelecionadas,
                        niveisSelecionados = s.niveisSelecionados
                    })
                    ;
                /*
        ...
        public PistaFullDto SelecionarPistaFull(int id)
        {

            var pista = this._unitOfWork.PistaRepository.GetById(id);
            var endereco = this._unitOfWork.EnderecoRepository.GetById(pista.id_endereco);
            var cidade = this._unitOfWork.CidadeRepository.GetById(endereco.id_cidade);
            var estado = this._unitOfWork.EstadoRepository.GetById(cidade.id_estado);
            var PistaModalidade = this._unitOfWork.ModalidadePistaRepository.Get(null, o => o.OrderBy(u => u.id_modalidade_pista)).Where(o => o.id_pista.Equals(pista.ID_pista)).Select(s => s.id_modalidade).ToList();
            var Modalidades = this._unitOfWork.ModalidadesRepository.Get().Where(o => PistaModalidade.Contains(o.id_modalidade)).Select(n => n.nm_modalidade);
            var PistaNiveis = this._unitOfWork.NiveisDificuldadePistaRepository.Get(null, o => o.OrderBy(u => u.id_nivel_dificuldade_pista)).Where(o => o.id_pista.Equals(pista.ID_pista)).Select(s => s.id_nivel_dificuldade).ToList();
            var Niveis = this._unitOfWork.NiveisDificuldadeRepository.Get().Where(o => PistaModalidade.Contains(o.id_nivel_dificuldade)).Select(n => n.nm_nivel_dificuldade);
            var Fotos = this._unitOfWork.FotosRepository.Get().Where(o => o.id_pista.Equals(pista.ID_pista)).Select(n => n.nm_foto);


            var Professorpista = this._unitOfWork.PistaProfessorRepository.Get(null, o => o.OrderBy(u => u.id_pista_professor)).Where(o => o.id_pista.Equals(pista.ID_pista)).Select(s => s.id_professor).ToList();
            //var Professores = this._unitOfWork.ProfessoresRepository.GetAll().Where(o => Professorpista.Contains(o.id_professor));
            var Professores = this._unitOfWork
                .ProfessoresRepository
                .Get(null, o => o.OrderBy(u => u.id_professor))
                ...
                .Where(o => Professorpista.Contains(o.id_professor))
                ;



            var PistaFull =  new PistaFullDto
                    {
        ...
        public PistaDto Selecionar(int id)
        {
            ...
            return new PistaDto
            {
                ...
                //modalidadesSelecionadas = this._unitOfWork.ModalidadePistaRepository.Get(null, o => o.OrderBy(u => u.id_modalidade_pista)).Where(o => o.id_pista.Equals(pista.ID_pista)).Select(s => s.id_modalidade).ToArray(),
                //niveisSelecionados = this._unitOfWork.NiveisDificuldadePistaRepository.Get(null, o => o.OrderBy(u => u.id_nivel_dificuldade_pista)).Where(o => o.id_pista.Equals(pista.ID_pista)).Select(s => s.id_nivel_dificuldade).ToList()
        ...
        public ResultadoDto Salvar(PistaCadastroUserDto pistaDto)
        {
            var pista = new Pistas();
            var endereco = new Endereco();
            var usuario_envio = new Usuario_envio();
